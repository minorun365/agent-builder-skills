...
> 実装もフロントのヘッダー送信も正しいのに、記録した利用者IDが全件「識別できず」になっていたら、Runtime 側の許可リスト未設定を疑う。
> 公式ドキュメントは「Pass custom headers to AgentCore Runtime」および runtime-oauth.html の Step 7 が該当。
